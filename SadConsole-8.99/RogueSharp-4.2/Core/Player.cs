...
namespace RogueSharpSamples.SadConsoleV8NetCore.Core
{
    public class Player : Actor
    {
        ...
        {
            statConsole.Print(1, 1, $"Name:    {Name}", Color.White);
            statConsole.Print(1, 3, $"Health:  {Health}/{MaxHealth}", Color.White);
            statConsole.Print(1, 5, $"Attack:  {Attack} ({AttackChance}%)", Color.White);
            statConsole.Print(1, 7, $"Defense: {Defense} ({DefenseChance}%)", Color.White);
            statConsole.Print(1, 9, $"Gold:    {Gold}", Color.Yellow);
        }
        ...
        private void DrawAbility(IAbility ability, Console inventoryConsole, int position)
        {
            ...
            if (ability.TurnsToRefresh > 0)
            {
                ...
                inventoryConsole.SetBackground(xHighlightPosition, yPosition, width, Swatch.DbOldBlood);
                inventoryConsole.SetBackground(xHighlightPosition + width, yPosition, remainingWidth, Color.Black);
            }
        }
