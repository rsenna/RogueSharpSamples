...
namespace GoRogueSample3.UI
{
    ...
    public class MessageLogWindow : Window
    {
            ...
            : base(width, height)
        {
            ...
            _messageConsole = new ScrollingConsole(width - WindowBorderThickness, MaxLines);
            _messageConsole.Position = new Point(1, 1);
            _messageConsole.ViewPort = new Rectangle(0, 0, width - 1, height - WindowBorderThickness);

            // create a scrollbar and attach it to an event handler, then add it to the Window
            _messageScrollBar = new ScrollBar(Orientation.Vertical, height - WindowBorderThickness);
        ...
        {
            _messageConsole.ViewPort = new Rectangle(0, _messageScrollBar.Value + WindowBorderThickness, _messageConsole.Width, _messageConsole.ViewPort.Height);
        }
        ...
        public override void Update(TimeSpan time)
        {
            ...
            if ((_messageConsole.TimesShiftedUp != 0) | (_messageConsole.Cursor.Position.Y >= _messageConsole.ViewPort.Height + _scrollBarCurrentPosition))
            {
                ...
                _messageScrollBar.Maximum = _messageConsole.Height + _scrollBarCurrentPosition - _messageConsole.Height - WindowBorderThickness;
